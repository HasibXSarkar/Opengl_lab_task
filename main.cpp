...
//rain();
        ...
        for (int i = 0; i < 5; i++) {
            ...
            float y = ((rand() % 200) / 100.0);
            float scale = ((rand() % 50) / 100.0) + 0.5;
            glPushMatrix();
            glTranslatef(x, y, 0.0f);
            glScalef(scale, scale, 1.0f);
            glColor3ub(255, 255, 242);
            thunder();
            glPopMatrix();
            glLoadIdentity();
        }
